...
func (r *userUsecase) Create(model dto.UserCreateValidation) error {
	user := models.User{}
...
func (r *userUsecase) Show(id string) models.User {
	return r.repo.Show(id)
}

func (r *userUsecase) Update(model dto.UserUpdateValidation) error {
	user := models.User{}
	err := smapping.FillStruct(&user, smapping.MapFields(&model))
	if err != nil {
		log.Fatalf("Failed map %v: ", err)
...
func (r *userUsecase) Delete(model models.User) error {
	return r.repo.Delete(model)
}

func (r *userUsecase) FindByEmail(email string) bool {
	return r.repo.FindByEmail(email)
}

func (r *userUsecase) Pagination(context *gin.Context, pagination *helpers.Pagination) response.Response {
